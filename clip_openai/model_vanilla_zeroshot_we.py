...
class CLIPDualEncoderModel(LightningModule):
    def __init__(
        ...
        if self.hparams.use_we:
            ...
            self.we_n = 0  # Initialize the counter for weight averaging
    ...
    def merge_we(self, model_new, model_old, sma_count):
        for param_new, param_old in zip(model_new.parameters(), model_old.parameters()):
            param_old.data = (param_old.data * sma_count + param_new.data) / (1.0 + sma_count)
        return model_old

    def on_train_batch_end(self, outputs, batch, batch_idx):
        # Perform weight averaging every `avg_freq` iterations after the batch is processed
        if self.hparams.use_we:
            if (self.global_step + 1) % self.hparams.we_avg_freq == 0:
                self.we_n += 1
                self.model_old = self.merge_we(self.model, self.model_old, self.we_n)
                # update weight
                for param_new, param_old in zip(self.model.parameters(), self.model_old.parameters()):
