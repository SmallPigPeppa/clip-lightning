...
run_training() {
  ...
  python3 cli_msun_s1.py fit \
    ...
    --trainer.logger.name ${METHOD}-${dataset}-${MODEL_NAME}-lr${lr}-bs${TOTAL_BATCH_SIZE}-alpha${ALPHA}-s1 \
    ...
    --model_checkpoint.filename ${METHOD}-${dataset}-${MODEL_NAME}-lr${lr}-bs${TOTAL_BATCH_SIZE}-alpha${ALPHA}-s1
}
