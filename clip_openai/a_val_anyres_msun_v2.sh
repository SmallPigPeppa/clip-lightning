...
python3 cli_val_anyres_msun_v2.py validate \
  --ckpt_path ${CKPT_DIR}/${METHOD}-${dataset}-${MODEL_NAME}-lr${lr}-bs${TOTAL_BATCH_SIZE}-alpha${ALPHA}-V2-v5.ckpt \
  --data.num_tasks 1 \
  --data.current_task 0 \
  --data.max_length 77 \
  ...
  --trainer.logger.name val-${METHOD}-${dataset}-${MODEL_NAME}-lr${lr}-bs${TOTAL_BATCH_SIZE}-alpha${ALPHA}-V2 \
  ...
  --model_checkpoint.filename ${METHOD}-${dataset}-${MODEL_NAME}-lr${lr}-bs${TOTAL_BATCH_SIZE}-alpha${ALPHA}-V2
