...
python3 cli_val_anyres.py validate \
  --ckpt_path ${CKPT_DIR}/${METHOD}-${dataset}-${MODEL_NAME}-lr${lr}-bs${TOTAL_BATCH_SIZE}-V2.ckpt \
  --data.num_tasks 1 \
  --data.current_task 0 \
  --data.max_length 77 \
  ...
  --trainer.logger.name val-${METHOD}-${dataset}-${MODEL_NAME}-lr${lr}-bs${TOTAL_BATCH_SIZE}-V2 \
