...
class CLIPDualEncoderModel(LightningModule):
    ...
    # def zscl_distill(self, image_features, text_features, image_features_old, text_features_old):
    #     # normalized features
    #     image_features = image_features / image_features.norm(dim=1, keepdim=True)
    #     text_features = text_features / text_features.norm(dim=1, keepdim=True)
    #
    ...
    #     # cosine similarity as logits
    #     logit_scale = self.model.logit_scale.exp()
    #     logits_per_image_old = logit_scale * image_features_old @ text_features_old.t()
    #     logits_per_text_old = logits_per_image_old.t()
    #
    #     # shape = [global_batch_size, global_batch_size]
    #
    #
    #     image_loss = F.cross_entropy(logits_per_image, logits_per_image_old)
    #     text_loss = F.cross_entropy(logits_per_text, logits_per_text_old)
    #
    #     loss_distill = (image_loss + text_loss) / 2
    #
    #
    #
    #     return loss_distill
    ...
    def training_step(self, batch, *args, **kwargs):
        ...
        if self.distill:
            ...
            return clip_loss + distill_loss * 0.04
        else:
            return clip_loss
    ...
    def validation_step(self, batch, *args, **kwargs):
        ...
        if self.distill:
            ...
            return clip_loss + distill_loss * 0.035
        else:
            return clip_loss
