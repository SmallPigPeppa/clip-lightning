...
class CLIPDualEncoderModel(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        parameters = [
            {
                "params": [p for n, p in self.model.named_parameters() if n.startswith("visual")],
                "lr": self.hparams.visual_lr,  # visual-specific learning rate
                "weight_decay": self.hparams.weight_decay
            },
            {
                "params": [p for n, p in self.model.named_parameters() if not n.startswith("visual")],
                "lr": self.hparams.text_lr,  # other parameters (text-related) learning rate
                "weight_decay": self.hparams.weight_decay
            }
        ]
        ...
        # Calculate the minimum of visual_lr and text_lr
        min_lr = min(self.hparams.visual_lr, self.hparams.text_lr)

        # Use the minimum learning rate for the LR scheduler
        lr_scheduler = LinearWarmupCosineAnnealingLR(
            optimizer,
            warmup_epochs=self.hparams.lr_warmup_epochs,
            max_epochs=self.trainer.max_epochs,
