...
class CLIPDualEncoderModel(LightningModule):
    ...
    def initialize_old_modules(self):
        if self.hparams.old_checkpoint_path is not None:
            if ',' in self.hparams.old_checkpoint_path:
                # 将逗号分隔的检查点路径拆分为列表
                checkpoint_paths = self.hparams.old_checkpoint_path.split(',')

                # 移除最后一个检查点路径
                last_checkpoint_path = checkpoint_paths.pop()
                ...
                last_params = last_checkpoint['model']
                ...
                # 将最终均值的参数加载到模型中
                self.model.load_state_dict(avg_params, strict=True)
                print("Model weights loaded successfully with averaged old and last checkpoint.")
