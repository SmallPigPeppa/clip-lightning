...
def modify_command_for_task_pretrain(base_command, params_dict, task_idx, num_tasks, sub_dir, pretrain_dir):
    """
    Modify the command dictionary for a specific task index and convert it back to a command string.
    """
    # Update parameters for incremental learning
    ...
    if task_idx > 0:
        # ckpt_dir = params_dict['--model_checkpoint.dirpath']
        # params_dict['--model.old_checkpoint_path'] = os.path.join(ckpt_dir, f"task-{task_idx - 1}.ckpt")
        params_dict['--model.old_checkpoint_path'] = os.path.join(pretrain_dir, f"task-{task_idx - 1}.ckpt")
    params_dict['--model_checkpoint.filename'] = f"task-{task_idx}"
...
def incremental_learning(init_task, num_tasks, script_path):
    """
    ...
    for task_idx in range(init_task, num_tasks):
        print(f"Starting training for task {task_idx}")
        if task_idx == init_task:
            pretrain_dir = "ckpt/simclr-5tasks"
            task_specific_command = modify_command_for_task_pretrain(base_command, params_dict.copy(), task_idx,
                                                                     num_tasks, sub_dir, pretrain_dir)
        else:
...
if __name__ == "__main__":
    ...
    script_path = "train_npu_r50_aug_bs256_debug_ILR_PLR_DILV2_distill_cassle_pretrain.sh"
    incremental_learning(init_task, num_tasks, script_path)
